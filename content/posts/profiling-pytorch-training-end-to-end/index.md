...
The original Markdown source for this post lives in the companion repo at [docs/pytorch_training_optimization_blog.md](https://github.com/duoan/playground-cuda/blob/main/docs/pytorch_training_optimization_blog.md). I bundled the generated artifacts here so the writeup stays self-contained.

The raw artifacts for this article are checked in:

- timing table: [timings.csv](./timings.csv)
- structured run summaries: [results.json](./results.json)
- charts live alongside this post bundle
- torch profiler outputs:
  - [blog_data_profile.txt](./blog_data_profile.txt)
  - [blog_torch_profile.txt](./blog_torch_profile.txt)
- Nsight Systems summary: [blog_torch_nsys_stats.txt](./blog_torch_nsys_stats.txt)
- Nsight Compute summary: [blog_kernel_ncu.txt](./blog_kernel_ncu.txt)
...
These are the steady-state numbers from [timings.csv](./timings.csv).
...
![Data Pipeline Experiments](./data_pipeline.svg)
...
![Data Pipeline Step-Time Jitter](./data_jitter.svg)
...
The strongest signals from [blog_data_profile.txt](./blog_data_profile.txt) were:
...
![Torch-Bound Throughput](./torch_throughput.svg)
...
The most useful lines from [blog_torch_profile.txt](./blog_torch_profile.txt) were:
...
The most useful findings from [blog_torch_nsys_stats.txt](./blog_torch_nsys_stats.txt) were:
...
![Kernel-Bound fp32 vs bf16](./kernel_amp.svg)
...
The key metrics from [blog_kernel_ncu.txt](./blog_kernel_ncu.txt) were:
